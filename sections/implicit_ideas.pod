...
from C<$_>.  Thus, these two lines of code are equivalent:
...
Perl passes arguments to functions in an array named C<@_>.  Array manipulation
operations (L<arrays>) performed inside functions affect this array by
...
functions.  Array functions such as C<unshift>, C<push>, C<splice>, and
C<pop> operate on C<@_>.

X<variables; @ARGV>
X<@ARGV>

Outside of a function, the default array variable is C<@ARGV>.  This array
represents all command-line arguments to the program.  The same array functions
which use C<@_> implicitly I<within> functions use C<@ARGV> implicitly at the
top level of a program outside of any function.  You cannot use C<@_> when you
mean C<@ARGV>.
