...
you've pre-declared a prototype for a function, you may use that function as a
...
Unlike other bareword examples, Perl's parser needs no disambiguation.  There
is only one way for it to interpret C<compare_lengths>.  Even so, the clarity
of explicit reference can help human readers.
